...
public class DriveCommands {
    ...
    return Commands.parallel(
        Commands.run(
                () -> {
                  ...
                  shooter.setFollow(isHubFollowing);

                  double desiredYawRad =
                      Math.atan2(
                          desiredPose.getY() - robotPose.getY(),
                          desiredPose.getX() - robotPose.getX());
                  ...
                  if (isHubFollowing) {
                    ...
                    Translation2d vecRobot = vecField.rotateBy(robotYaw.unaryMinus());
                    ...
                    tangentialInput =
                        MathUtil.applyDeadband(
                            -ySupplier.getAsDouble() * DriveConstants.supplierPercent,
                            DriveConstants.DEADBAND);
                    tangentialInput =
                        Math.copySign(tangentialInput * tangentialInput, tangentialInput);
                    double tangentialSpeed =
                        tangentialInput * drive.getMaxLinearSpeedMetersPerSec();
                    Translation2d tangentialVelRobot = tangentialUnitRobot.times(tangentialSpeed);

                    // Sum velocities in robot frame
                    linearVelocity = radialVelRobot.plus(tangentialVelRobot);
                    ...
                    // Square rotation value for more precise control
                    omega = MathUtil.applyDeadband(omega, 0.4);
                    ...
                    Rotation2d rawBase = drive.getGyroRelativeRotationRaw();
                    Rotation2d baseRot =
                        isFlipped ? rawBase.plus(new Rotation2d(Math.PI)) : rawBase;
  ...
  public static Command driveToHubWithTimeout(Drive drive, Shooter shooter, double seconds) {
    ...
    return Commands.parallel(
            Commands.run(
                    () -> {
                      ...
                      // Vector desde robot hasta hub (campo)
                      Translation2d vecField =
                          hubPose.getTranslation().minus(robotPose.getTranslation());
                      double dist = vecField.getNorm();
                      if (dist < 1e-6) {
                        drive.runVelocity(new ChassisSpeeds(0, 0, 0));
                        return;
                      ...
                      if (Math.abs(radialError) > DriveConstants.radialToleranceMeters) {
                        ...
                        // Dentro de tolerancia: evitar micro-correcciones/ruido y resetear
                        // integrador
                        radialPID.reset();
                        radialSpeed = 0.0;
                      }
                      ...
                      // Angular control: face the hub (compute desired yaw in field then convert to
                      // gyro
                      // frame)
                      double desiredYawRad =
                          Math.atan2(
                              hubPose.getY() - robotPose.getY(), hubPose.getX() - robotPose.getX());
                      // if (isHub2 && PositionConstants.HUB2_INVERT_FACING) desiredYawRad +=
                      // Math.PI;
                      double desiredYawRel = desiredYawRad - drive.getGyroZero().getRadians();
                      double currentYawRel = robotYaw.getRadians();
                      ...
                      double omega =
                          angleController.calculate(
                              currentYawRel,
                              desiredYawRel
                                  - (tangentialInput * DriveConstants.angleHeadingRadians));
  ...
  public static Command ChangeFollowing(Drive drive) {
    return Commands.runOnce(
        () -> {
          isHubFollowing = !isHubFollowing;
          drive.UpdateFollowingSatus(isHubFollowing);
        });
  }
